...
module Text.Liquor.Parser where
...
import qualified Text.Liquor.Interpreter as I
...
plain :: I.Plain :<: s => Parser (I.Statement e s)
...
if_ :: (I.ShopifyStatementSuper e s, I.ShopifyExpressionSuper e) => Parser (I.Statement e s) -> Parser (I.Statement e s)
if_ stmt = do
  c <- tagWith ifStart expression
  t <- template' stmt
  e <- elseList stmt <|> pure []
  _ <- tag ifEnd
  pure $ I.if_ $ (c, t):e

elseList :: (I.ShopifyStatementSuper e s, I.ShopifyExpressionSuper e) => Parser (I.Statement e s) -> Parser [(I.Expression e, [I.Statement e s])]
elseList stmt =
  do
    _ <- tag else_
    t <- template' stmt
...
for :: (I.ShopifyStatementSuper e s, I.ShopifyExpressionSuper e) => Parser (I.Statement e s) -> Parser (I.Statement e s)
for stmt = do
...
output :: (I.Output e :<: s, I.ShopifyExpressionSuper e) => Parser (I.Statement e s)
output = I.output <$> outputTag ({-filterBlock <|>-} expression)

statement :: (I.ShopifyStatementSuper e s, I.ShopifyExpressionSuper e) => Parser (I.Statement e s) -> Parser (I.Statement e s)
statement stmt =
  choice
    [ output
    , if_ stmt
...
template' :: (I.ShopifyStatementSuper e s, I.ShopifyExpressionSuper e) => Parser (I.Statement e s) -> Parser (I.Template e s)
template' stmt =
  endOfInput *> pure []
  <|> (lookAhead (char '{') *> ((:) <$> stmt <*> template' stmt <|> pure []))
  <|> (:) <$> plain <*> template' stmt

template :: (I.ShopifyStatementSuper e s, I.ShopifyExpressionSuper e) => Parser (I.Template e s)
template =
